...
For example, to request the CMDI Data View from an Endpoint that has an Endpoint Description, as described in <<gh-dv-cmdi-resource>>, a Client would need to use the _Data View identifier_ `cmdi` and submit the following request:
